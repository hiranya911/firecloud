...
readonly LAST_TAG=`git describe --tags $(git rev-list --tags --max-count=1) 2> /dev/null` || true
if [[ -z "${LAST_TAG}" ]]; then
  echo "[INFO] No tags found. Including all commits up to ${GITHUB_SHA}."
  readonly VERSION_RANGE="${GITHUB_SHA}"
else
  echo "[INFO] Last release tag: ${LAST_TAG}"
  readonly COMMIT_SHA=`git show-ref -s ${LAST_TAG}`
  echo "[INFO] Last release commit: ${COMMIT_SHA}"
  readonly VERSION_RANGE="${COMMIT_SHA}..${GITHUB_SHA}"
